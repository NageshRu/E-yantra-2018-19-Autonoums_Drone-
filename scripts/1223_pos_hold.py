...
class DroneFly():
	...
	def __init__(self):
		...
		self.waypoints=[( -0.1, -2.7, 14),( 4.8, -5.3, 16),( 5.0, 0.3, 18),( -5.2, 5, 20), (-0.6, 5.70, 20)]
	...
	* Example Call: self.calc_pid()
	...
	def calc_pid(self):
		...
		if(self.current_time >= self.loop_time):
			...
			if(abs(self.throt_error) <2):
				_,_,self.wp_z=self.waypoints[self.i]



			if(abs(self.roll_error)<1.5 and abs(self.pitch_error)<1.5 and ): 
